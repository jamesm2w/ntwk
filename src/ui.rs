...
use crate::canvas::mode::{Mode, PlaceEdgeProgress};
...
impl Sandbox for NetworkUI {
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        Column::new()
            ...
            .push(
                Row::new().padding(20).align_items(Alignment::Fill)
                .push(Button::new("Add Node").on_press(Message::ChangePenMode(Mode::PlaceNode)))
                .push(Button::new("Add Edge").on_press(Message::ChangePenMode(Mode::PlaceEdge(PlaceEdgeProgress::None))))
                .push(Button::new("Clear").on_press(Message::Clear))
            )
            .push(
                Canvas::new(NetworkCanvas::new(&self.graph, &self.canvas_cache, &self.pen_mode))
